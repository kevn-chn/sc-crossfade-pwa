...
if (/#\/*callback$/.test(window.location.href)) {
  if (window.opener) {
    window.onload = () => window.setTimeout(window.opener.SC.connectCallback, 1);
  }

  const el = document.createElement('p');
  el.style = 'text-align: center; padding: 16px;';
  el.innerHTML = 'This pop-up should automatically close in a few seconds...';
  document.body.appendChild(el);
  document.title = 'Connect with SoundCloud';
} else {
  SC.initialize({
    client_id: process.env.ELM_APP_SC_API_CLIENT_ID,
    redirect_uri: process.env.NODE_ENV === 'development'
      ? 'http://location:3000/#/callback'
      : 'https://kevn-chn.github.io/sc-crossfade-pwa/#/callback',
  });
